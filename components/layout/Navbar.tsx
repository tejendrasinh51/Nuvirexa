    ...
    <>
      ...
      <motion.header
        ...
        className={cn(
          ...
          scrolled
            ? 'bg-background/80 backdrop-blur-2xl border-b border-white/[0.08] py-3 shadow-[0_1px_0_rgba(255,255,255,0.04)]'
            : 'bg-transparent py-5'
        )}
      >
        <nav className="container mx-auto flex items-center justify-between safe-top" aria-label="Main navigation">
          <Link href="/" className="flex items-center gap-2 group">
            <span className="font-display font-black text-xl text-white tracking-tight group-hover:text-gradient transition-all">
              Nuvirexa
            </span>
            <motion.div
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className="relative text-sm font-medium text-white/70 hover:text-white transition-colors group py-1"
                >
                  {link.label}
                  <span
                    className={cn(
